...
func TestEach(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var result []int
			slicekit.Each(test.source, func(elem int) {
				result = append(result, elem)
			})

			if !slicekit.Equals(result, test.expected, IntEqual) {
...
func TestReverseEach(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var result []int
			slicekit.ReverseEach(test.source, func(elem int) {
				result = append(result, elem)
			})
...
func TestEachWithModification(t *testing.T) {
	...
	source := []int{1, 2, 3, 4, 5}
	var sum int
	slicekit.Each(source, func(elem int) {
		sum += elem
	})
...
func TestReverseEachWithModification(t *testing.T) {
	...
	source := []int{1, 2, 3, 4, 5}
	var sum int
	slicekit.ReverseEach(source, func(elem int) {
		sum += elem
	})
